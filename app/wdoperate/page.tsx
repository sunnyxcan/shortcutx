...
import { Play, Trash2, Copy, Check, FileText, ArrowLeft, Layers, UserCheck } from 'lucide-react';
import Link from 'next/link';

interface AutoWDTransaction {
  bank: string;
  account: string;
  username: string;
  realname: string;
...
export default function AutoWDParser() {
  const [input, setInput] = useState('');
  const [transactions, setTransactions] = useState<AutoWDTransaction[]>([]);
  ...
  const handleProcess = async () => {
    ...
    try {
      const res = await fetch('/api/autowd', {
        method: 'POST',
  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-hidden bg-slate-950 text-slate-200 md:flex-row">
      <aside className="relative flex w-full flex-shrink-0 flex-col border-b border-slate-800 bg-slate-900/50 p-6 backdrop-blur-xl md:w-[400px] md:border-b-0 md:border-r">
        <div className="mb-8">
          <Link href="/" className="group mb-6 inline-flex items-center gap-2 text-xs font-medium text-slate-500 transition-colors hover:text-emerald-400">
            <ArrowLeft className="h-3 w-3 transition-transform group-hover:-translate-x-1" />
            KEMBALI KE DASHBOARD
          </Link>
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-500 shadow-inner shadow-emerald-500/20">
        ...
        <div className="relative flex flex-1 flex-col">
          ...
          <textarea 
            ...
            className="flex-1 resize-none rounded-xl border border-slate-800 bg-black/40 p-4 font-mono text-xs leading-relaxed text-slate-300 outline-none placeholder:text-slate-700 focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50"
            placeholder="Tempel data penarikan mentah di sini..."
            spellCheck={false}
          />
        </div>
        ...
        <div className="mt-6 flex gap-3">
          <button 
            ...
            className="flex flex-[2] items-center justify-center gap-2 rounded-xl bg-emerald-600 py-3.5 text-sm font-bold text-white shadow-lg shadow-emerald-900/20 transition-all hover:bg-emerald-500 hover:shadow-emerald-900/40 disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none cursor-pointer"
          >
            {loading ? <Layers className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4 fill-current" />}
        ...
        <div className="z-10 flex h-full flex-col p-6 md:p-10">
          ...
          {hasProcessed ? (
                    ...
                    <tbody className="divide-y divide-slate-800/50 text-sm">
                      {transactions.map((trx, i) => (
                        <tr key={i} className="group transition-colors hover:bg-slate-800/50">
                          ...
                          <td className="px-4 py-3 text-right font-mono font-medium text-emerald-400">{formatRupiah(trx.amount)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="flex flex-1 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-800 bg-slate-900/20 text-center">
              ...
              <p className="mt-2 max-w-xs text-sm text-slate-500">Tempel daftar penarikan Anda untuk menghasilkan format auto-payout.</p>
            </div>
          )}
        </div>
      </main>
