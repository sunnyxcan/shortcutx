...
import { Play, Trash2, Copy, Check, FileText, ArrowLeft, RefreshCw } from 'lucide-react';
...
export default function BcaParser() {
  ...
  const handleCopy = () => {
    ...
    const text = transactions.map(t => `${t.name}\t${t.amount}`).join('\n');
  ...
  return (
    <div className="flex h-screen w-full flex-col overflow-hidden bg-slate-950 text-slate-200 md:flex-row">
      <aside className="relative flex w-full flex-shrink-0 flex-col border-b border-slate-800 bg-slate-900/50 p-6 backdrop-blur-xl md:w-[400px] md:border-b-0 md:border-r">
        <div className="mb-8">
          <Link href="/" className="group mb-6 inline-flex items-center gap-2 text-xs font-medium text-slate-500 transition-colors hover:text-blue-400">
            <ArrowLeft className="h-3 w-3 transition-transform group-hover:-translate-x-1" />
            KEMBALI KE DASHBOARD
          </Link>
          <div className="flex items-center gap-4">
            <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-blue-500/10 text-blue-500 shadow-inner shadow-blue-500/20">
              <RefreshCw className="h-6 w-6" />
            </div>
            <div>
              <h1 className="text-xl font-bold text-white">Mutasi <span className="text-blue-500">BCA</span></h1>
              <p className="text-xs text-slate-400">Parser Mutasi BCA</p>
            </div>
          </div>
        </div>
        ...
        <div className="relative flex flex-1 flex-col">
          <div className="mb-2 flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-600">Tempel data mutasi bca di bawah</span>
          </div>
          <textarea 
            ref={inputRef}
            value={input}
            onChange={(e) => setInput(e.target.value)}
            className="flex-1 resize-none rounded-xl border border-slate-800 bg-black/40 p-4 font-mono text-xs leading-relaxed text-slate-300 outline-none placeholder:text-slate-700 focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50"
            placeholder="Tempel data mutasi mentah di sini..."
            spellCheck={false}
          />
        </div>
        ...
        <div className="mt-6 flex gap-3">
          <button 
            ...
            className="flex flex-[2] items-center justify-center gap-2 rounded-xl bg-blue-600 py-3.5 text-sm font-bold text-white shadow-lg shadow-blue-900/20 transition-all hover:bg-blue-500 hover:shadow-blue-900/40 disabled:cursor-not-allowed disabled:opacity-50 disabled:shadow-none cursor-pointer"
          >
        ...
        <div className="z-10 flex h-full flex-col p-6 md:p-10">
          <header className="mb-8 flex flex-shrink-0 items-end justify-between border-b border-slate-800 pb-6">
            <div>
              ...
              <p className="text-sm text-slate-500">Data transaksi yang telah diproses</p>
            </div>
            {hasProcessed && (
               <button 
               onClick={handleCopy}
          ...
          {hasProcessed ? (
                ...
                <div className="h-full overflow-auto custom-scrollbar">
                  <table className="w-full text-left">
                    <thead className="sticky top-0 z-20 bg-slate-900 text-xs font-bold uppercase tracking-wider text-slate-500">
                      <tr>
                        <th className="w-16 border-b border-slate-800 px-6 py-4">No</th>
                        <th className="border-b border-slate-800 px-6 py-4">Nama Pengirim</th>
                        <th className="border-b border-slate-800 px-6 py-4 text-right">Nominal</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-800/50 text-sm">
                      {transactions.map((trx, i) => (
                        <tr key={i} className="group transition-colors hover:bg-slate-800/50">
                          <td className="px-6 py-4 font-mono text-slate-500 group-hover:text-slate-400">{i + 1}</td>
                          <td className="px-6 py-4 font-medium text-slate-300 group-hover:text-white">{trx.name}</td>
                          <td className="px-6 py-4 text-right font-mono font-medium text-blue-400">{formatRupiah(trx.amount)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="flex flex-1 flex-col items-center justify-center rounded-2xl border border-dashed border-slate-800 bg-slate-900/20 text-center">
              ...
              <p className="mt-2 max-w-xs text-sm text-slate-500">Tempel data mutasi Anda di panel kiri lalu klik proses untuk menganalisis.</p>
            </div>
          )}
        </div>
      </main>
